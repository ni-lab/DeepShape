    ...
    def create_seqlets(self, seqlets):
        for seqlet in seqlets:
            ...
            if seqlet.is_revcomp:
                if self.nofliprc:
                    seqlet.sequence = self.one_hot[idx][s:e][::-1]
                    seqlet.contrib_scores = self.contrib_scores[idx][s:e][::-1]
                    seqlet.hypothetical_contribs = self.hypothetical_contribs[idx][s:e][::-1]
                else:
...
class Seqlet(object):
    ...
    def revcomp(self, nofliprc=False):
        new_seqlet = Seqlet(
                example_idx=self.example_idx,
                start=self.start, end=self.end,
                is_revcomp=(self.is_revcomp==False))
        if nofliprc:
            new_seqlet.sequence = self.sequence[::-1]
            new_seqlet.contrib_scores = self.contrib_scores[::-1]
            new_seqlet.hypothetical_contribs = self.hypothetical_contribs[::-1]
        else:
