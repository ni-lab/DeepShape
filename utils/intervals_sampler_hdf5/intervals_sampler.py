...
class IntervalsSampler(OnlineSampler):
	...
	def sample(self, batch_size=1, mode=None):
		...
		if hasattr(self.reference_sequence, 'n_shape_features'):
			enc_dim = 4 + self.reference_sequence.n_shape_features
		else:
			enc_dim = 4
		sequences = np.zeros((batch_size, self.sequence_length, enc_dim)) #modified from 4
